...
async function main() {
   const canvas = document.getElementById("glCanvasCenario");
   ...
   // Identifica quais faixas (Y) são ruas (onde tem carros)
   const roadLanes = new Set();
   cars.forEach(c => roadLanes.add(c.y));

   // Gera o cenário de Y = -15 até Y = 30 (cobre boa parte do percurso)
   for (let y = -15; y <= 50; y += 1) {
       if (roadLanes.has(y)) {
           // É uma rua -> Adiciona modelo de Estrada Longa (ou pequena repetida)
           // Usando long_road (índice 8)
           scenery.push({
               x: 0, // Altura do chão
               y: y, // Faixa
               z: 0, // Centralizado horizontalmente
               scale: 1.0, 
               modelIndex: 8, // long_road.obj
               rotationY: 90 // Rotação para alinhar com a pista (ajuste se necessário: 0 ou 90)
           });
       } else {
           // É grama -> Preenche a linha horizontalmente com blocos de grama
           // Z vai de -5 a 5 para cobrir a tela larga
           for (let z = -5; z <= 5; z += 1) {
               scenery.push({
                   x: 0,
                   y: y,
                   z: z,
                   scale: 1.0, // Ajuste para o tamanho do bloco
                   modelIndex: 7, // grass.obj
                   rotationY: 0
               });

               // Chance de 10% de aparecer um Boneco de Neve (Neves/Obstáculo)
               // Evita colocar exatamente onde o jogador começa (-12, 0)
               if (Math.random() < 0.1 && !(y === player.y && z === player.z)) {
                   scenery.push({
                       x: 0,
                       y: y,
                       z: z,
                       scale: 0.5,
                       modelIndex: 9   , // snowman.obj
                       rotationY: Math.random() * 360 // Rotação aleatória
                   });
               }
           }
       }
   ...
   function drawScene(time) {
      ...
      // 1. Desenha o Cenário (Grama, Ruas, Árvores/Neves) - PRIMEIRO para ficar no fundo
      scenery.forEach((obj) => {
          drawObj(obj);
      });
